...
const INTERVAL_MIN = parseInt(getArg("--interval") ?? "5", 10);
...
async function main() {
  ...
  const totalSteps = Math.floor(
    (now.getTime() - startDate.getTime()) / intervalMs,
  );
  ...
  console.log(
    `  Points     : ${totalSteps} par capteur (${totalSteps * 5} total)`,
  );
  ...
  for (let i = 0; i < totalSteps; i++) {
    ...
    records.push({
      createdAt: timestamp,
      value: state.humidity.toFixed(2),
    ...
    records.push({
      createdAt: timestamp,
      value: Math.round(state.co2).toString(),
      type: DataType.CO2,
    });
    records.push({
      createdAt: timestamp,
      value: Math.round(state.light).toString(),
      type: DataType.LIGHT,
    });
  }
