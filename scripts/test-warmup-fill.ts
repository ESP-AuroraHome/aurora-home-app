...
async function main() {
  ...
  const rows = [];
  for (const [type, range] of Object.entries(SENSOR_RANGES) as [DataType, { min: number; max: number }][]) {
    for (let i = 0; i < POINTS_TO_INSERT; i++) {
      const value = (range.min + Math.random() * (range.max - range.min)).toFixed(2);
      rows.push({
        type,
        value,
  ...
  console.log(`✅ ${rows.length} DataPoints insérés (${POINTS_TO_INSERT} par capteur)`);
  console.log("   Rechargez le dashboard — le banner de chauffe doit avoir disparu.");
}
