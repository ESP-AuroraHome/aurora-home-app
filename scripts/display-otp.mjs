...
function writeLine(bus, page, text, invert = false) {
  setPage(bus, page);
  let cols = renderText(text);
  // Pad or truncate to exactly 128 columns
  if (cols.length < 128) cols = cols.concat(new Array(128 - cols.length).fill(0));
...
function writeLineLarge(bus, startPage, text) {
  ...
  if (padded.length < 128) padded.push(...new Array(128 - padded.length).fill(0));
...
if (!command) {
  console.error("Usage: display-otp.mjs show <otp> <email> | display-otp.mjs clear");
  process.exit(1);
}
